"""Script temporal para regenerar Gutran_relation_generator.py correctamente"""

content = '''# functions/Gutran_relation_generator.py
"""
...
def generate_gutran_relation_mos(rnd_file_xlsx: Any, nemonico: str, release: str) -> str:
        ...
        for idx in range(maxcols):
            ...
            for attr_name in GUTRANFREQRELATION_SET_ORDER:
                ...
                if attr_name == "gUtranSyncSignalFrequencyRef":
                    if key_match:
                        attrs_present.pop(key_match, None)
                    lines.append(f"set ENodeBFunction=1,EUtranCellFDD={cell},GUtranFreqRelation={freqrel} gUtranSyncSignalFrequencyRef ")
                    continue
    ...
    if "ExternalGUtranCell" in data:
        ...
        first_key = next(iter(extcell_map.keys()))
        ids = extcell_map[first_key]
        ...
        for idx in range(len(ids)):
            ...
            for a, vlist in extcell_map.items():
                ...
                if v == "":
                    continue
                low = a.strip().lower()
                if "enodebfunction" in low or "externalgutrancell" in low:
                    continue
                if "abssubframeoffset" in low:
                    lines.append(f"absSubFrameOffset {v}")
                elif "abstimeoffset" in low:
                    lines.append(f"absTimeOffset {v}")
                elif "gutransyncsignal" in low or "gutransync" in low:
                    lines.append(f"gUtranSyncSignalFrequencyRef {v}")
                elif "isremoveallowed" in low:
                    lines.append(f"isRemoveAllowed {v}")
                elif "localcellid" in low:
                    lines.append(f"localCellId {v}")
                elif "plmnidlist" in low:
                    lines.append(f"plmnIdList {v}")
                else:
                    lines.append(f"{a.strip()} {v}")
            lines.append("end")
            lines.append("")
    ...
    if "GUtranCellRelation" in data:
        ...
        for idx in range(maxcols):
            ...
            for other_attr, vals in sorted(attrs_present.items()):
                ...
                if val == "":
                    continue
                low = other_attr.strip().lower()
                if low.endswith("id") or "_id" in low:
                    continue
                if any(attr.lower() in low for attr in GUTRANCELLRELATION_SET_ORDER):
                    continue
                lines.append(f"{other_attr.strip()} {val}")
...
# Escribir el archivo
with open(r'Macro\functions\Gutran_relation_generator.py', 'w', encoding='utf-8') as f:
    f.write(content)

print("Archivo regenerado exitosamente")
