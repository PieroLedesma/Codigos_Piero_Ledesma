# =====================================================================
# generator_logic.py (VERSIÓN FINAL, COMPLETA Y OPTIMIZADA)
# Compatible con UTRAN, RRUS, Celdas y Terreno
# =====================================================================
...
def generar_archivos_zip(
    ...
    # ======================================================
    # 2) LEER EXCEL RND COMPLETO
    # ======================================================
    try:
        rnd_excel = pd.ExcelFile(rnd_file_xlsx, engine='openpyxl')

        required_sheets = ['Node', 'Cell-Carrier', 'Equipment-Configuration']
        if not all(s in rnd_excel.sheet_names for s in required_sheets):
            return None, (
                "Error: El RND debe contener las hojas: "
                "'Node', 'Cell-Carrier' y 'Equipment-Configuration'"
            ), None

        df_cell = pd.read_excel(rnd_excel, sheet_name='Cell-Carrier')
        df_cell.columns = df_cell.columns.str.strip()

    except Exception as e:
        return None, f"Error al leer RND XLSX: {e}", None
...
<ConfigurationFiles
    ...
    return zip_buffer.getvalue(), f"Scripts_{nem}.zip", generated_content
